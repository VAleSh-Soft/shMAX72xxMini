...
Для работы с **ESP32C3** матрица подключается к пинам **FSPI** модуля, а именно:
...
Для работы с **ESP32S3** и **ESP32S2** матрица подключается к пинам:
- **FSPI**
  - **DataIn** (**DIN**) - к пину **MOSI** (**GPI11**);
  - **CLK** - к пину **SCK** (**GPI12**);
  - **CS**(**LOAD**) - к пину **GPIO9**; **ВАЖНО** - это не должен быть пин **GPI10**;
- **HSPI** - этот интерфейс не имеет пинов по умолчанию, поэтому работа матрицы тестировалась на следующих пинах:
