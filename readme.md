...
- добавлен метод `setBrightnessForAllDevices(uint8_t intensity)`, позволяющий установить яркость свечения светодиодов для всех устройств одновременно;
- обновлены примеры;
...
`void shutdownAllDevices(bool off_device)` - то же самое, но для всех устройств в каскаде одновременно;
