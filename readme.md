...
+ добавлен метод `void setSPI(shSPIClass *spi);`, позволяющий переключить интерфейс **SPI** для вывода данных;
...
+ добавлен метод `void init(int8_t clk_pin, int8_t din_pin);`, позволяющий задать соответствующие пины; метод доступен для **RP2040**;
