...
+ добавлен метод `void setSPI(shSPIClass *spi);`, позволяющий переключить интерфейс **SPI** для вывода данных; метод доступен для **RP2040**;
...
+ добавлен метод `void init(int8_t clk_pin, int8_t din_pin);`, позволяющий задать соответствующие пины; метод доступен для **ESP32**, **RP2040**;
...
Для работы с **ESP32** матрица подключается к пинам **VSPI** модуля, а именно:
- **DataIn** (**DIN**) - к пину **MOSI** (**GPIO23**);
- **CLK** - к пину **CLK** (**GPIO18**);
- **CS**(**LOAD**) - к пину **GPIO4**;
