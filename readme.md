...
Метод доступен для МК, позволяющих переназначать пины **SPI**-интерфейсов.
...
`void setSPI(shSPIClass *spi)` - метод задает **SPI**-интерфейс для вывода данных; метод доступен для МК, имеющих более одного **SPI**-интерфейса (**ESP8266**, **ESP32**, **RP2040**, **STM32**); здесь:
...
Для работы матрицы на интерфейсе **SPI1** (с назначением этого интерфейса, см. описание метода `setSPI1()`) нужно подключить ее к cоответствующим пинам:
...
Для работы с **RP2040** матрица подключается к пинам **SPI0** модуля, а именно:
- **DataIn** (**DIN**) - к пину **TX** (**GPIO3**);
- **CLK** - к пину **SCL** (**GPIO2**);
- **CS**(**LOAD**) - к пину **CS** (**GPIO1**);

#### STM32

Протестировано подключение по умолчанию, без переназначения пинов. Библиотека тестировалась на **STM32F103C8T6** (**BluePill**)

Для работы с **BluePill** матрица подключается к пинам **SPI1** модуля, а именно:
- **DataIn** (**DIN**) - к пину **MOSI1** (**A7**);
- **CLK** - к пину **SCK1** (**A5**);
