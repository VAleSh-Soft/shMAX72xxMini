...
- [Подключение драйвера к Ардуино](#подключение-драйвера-к-ардуино)
  ...
  - [RP2040](#rp2040)
  - [STM32](#stm32)
- [Аддоны, используемые для работы со сторонними платами](#аддоны-используемые-для-работы-со-сторонними-платами)
...
+ добавлен метод `void init(uint8_t spi = SPI_SPI0)`, предназначенный для инициализации интерфейса на **ATmega328pb** с возможностью выбора одного из двух доступных **SPI**;
+ добавлены примеры одновременного использования двух **SPI**-интерфейсов на примере одновременного вывода двух бегущих строк; примеры написаны для **ATmega328pb**, **ESP32**, **STM32 Blue/Black Pill** и **RP2040**;
+ метод `setSPISettings()` заменен методом `setSPIFrequency(uint32_t frequency)`, т.к. частота работы **SPI**-интерфейса - это единственное, что имеет смысл менять для драйвера **MAX7219/7221**;
* доработан пример **TwoTickersForESP32.ino** для работы на **ESP32S2**, **ESP32S3**, **ESP32C3**, **ESP32C6** и **ESP32H2**;
+ добавлена возможность вертикального смещения столбцов и горизонтального смещения строк при выводе на экран;
* доработан пример **MatrixDemo.ino** для демонстрации смещения данных;
* в классе `shMAX72xx7Segment` добавлен метод `setChar(uint8_t addr, uint8_t num_index, uint8_t value, bool showDot, bool upd)`;
* исправлены опечатки в документации и `keywords.txt`;
* оптимизированы и ускорены методы `update()` и `clearAllDevices()`;
...
***Важно!!!** - максимальное количество 127 устройств в модуле имеет смысл только для МК с большим объемом динамической памяти; обычная **Arduino UNO/nano/pro mini** такое количество не потянет.*
...
`void setSPIFrequency(uint32_t frequency)` - настройка частоты **SPI** для работы драйвера; здесь:
- `frequency` - частота работы **SPI**, Гц; 

`void shutdownDevice(uint8_t addr, bool off_device)` - перевод в режим сна или вывод из него устройства с индексом **addr**; здесь:
- `off_device` - определяет действие: при **true** переводит устройство в режим сна (при этом информация в его регистрах сброшена не будет), при **false** - включает его;
...
`void clearAllDevices(bool upd = false)` - то же самое, но для всех устройств каскада сразу.
...
`uint8_t getLedState(uint8_t addr, uint8_t row, uint8_t column)` - получить состояние одиночного светодиода на устройстве; здесь: 
- `addr` - индекс устройства в каскаде;
- `row` - строка (координата **Y**);
- `column` - столбец, (координата **X**); 
...
`void setRow(uint8_t addr, uint8_t row, uint8_t value, int8_t offset, bool upd)` - установка строки на устройстве; здесь:
- `addr` - индекс устройства в каскаде;
- `row` - строка (0..7);
- `value` - битовая маска строки; состояние каждого светодиода строки определяется значением соответствующего бита в параметре `value`: 1 - включен, 0 - выключен;
- `offset` - горизонтальное смещение строки, значение -8..8; при `offset = 0` строка выводится с первой колонки, при `offset < 0` строка смещается влево, при `offset > 0` строка смещается вправо
- `upd` - признак обновления устройства; при `upd = false`, то будет очищен только программный буфер устройства, само изображение будет обновлено только после вызова метода `update()`.

Метод учитывает заданные поворот и отражение изображения.

`uint8_t getRow(uint8_t addr, uint8_t row)` - получение битовой маски отображаемой строки; здесь:
- `addr` - индекс устройства в каскаде;
- `row` - строка (0..7);
...
`void setColumn(uint8_t addr, uint8_t column, uint8_t value, int8_t offset, bool upd)` - установка столбца на устройстве с учетом вертикального смещения; здесь:
- `addr` - индекс устройства в каскаде;
- `column` - столбец (0..7);
- `value` - битовая маска строки; состояние каждого светодиода строки определяется значением соответствующего бита в параметре `value`: 1 - включен, 0 - выключен;
- `offset` - вертикальное смещение столбца, значение -8..8; при `offset = 0` столбец выводится с первой строки, при `offset < 0` столбец смещается вверх, при `offset > 0` столбец смещается вниз;
- `upd` - признак обновления устройства; при `upd = false`, то будет очищен только программный буфер устройства, само изображение будет обновлено только после вызова метода `update()`.

**ВАЖНО!!! - имейте в виду, что используя этот вариант метода `setColumn()`, нужно обязательно указывать параметр `upd`**.

Метод учитывает заданные поворот и отражение изображения.

`uint8_t getColumn(uint8_t addr, uint8_t column)` - получение битовой маски отображаемого столбца; здесь:
- `addr` - индекс устройства в каскаде;
- `column` - столбец (0..7);

Метод учитывает заданные поворот и отражение изображения.

`void update()` - обновление изображения (вывод содержимого программного буфера) на всех устройствах модуля; выполняется за 8 **SPI**-посылок независимо от количества устройств в каскаде;
...
- `digit` - число, маску которого нужно получить, может задаваться в интервале 0-15; позволяет получать битовые маски цифр 0-9 и символов A, b, C, d, E и F;
...
`void setChar(uint8_t addr, uint8_t num_index, uint8_t value, bool showDot = false, bool upd = false);` - вывод битовой маски на цифровой индикатор; отличие от первого метода, в методе указывается индекс устройства и индекс знака на устройстве; здесь:
- `addr` - индекс устройства в каскаде, начиная с нуля;
- `num_index` - индекс знака в устройстве, начиная с нуля;
...
На данный момент библиотека тестировалась в работе на **ATmega168p/328p/328pb**, **esp8266**, **esp32**, **esp32c3**, **esp32c6**, **esp32s2**, **esp32s3**, **esp32h2**, **rp2040** (**Zero** и **Pico**), **stm32f103c8t6** (**Blue Pill**), **stm32f401ccu6** (**Black Pill**), **stm32f411ceu6** (**Black Pill**). 

Работа с интерфейсом, отличным от интерфейса по умолчанию (**SPI**), тестировалась на **ATmega328pb**, **esp32**, **esp32c3**, **esp32c6**, **esp32s2**, **esp32s3**, **rp2040**, **stm32f103c8t6** (**Blue Pill**), **stm32f401ccu6** (**Black Pill**), **stm32f411ceu6** (**Black Pill**), **stm32f411ceu6** (**Black Pill**). 

Переназначение пинов **SPI**-интерфейса протестировано на **esp32**, **esp32c3**, **esp32c6**, **esp32s2**, **esp32s3**, **esp32h2**, **rp2040**.
...
Для работы драйвера нужно подключить его к пинам **SPI**:
...
Подключения по умолчанию, без переназначения пинов.
...
Для работы с **ESP32C3**, **ESP32C6** и **ESP32H2** драйвер подключается к пинам **FSPI** модуля, а именно:
- **ESP32C3**
  - **DataIn** (**DIN**) - к пину **FSPI_MOSI** (**GPIO6**);
  - **CLK** - к пину **FSPI_SCK** (**GPIO4**);
  - **CS**(**LOAD**) - к пину **GPIO7**;
- **ESP32C6**
  - **DataIn** (**DIN**) - к пину **FSPI_MOSI** (**GPIO19**);
  - **CLK** - к пину **FSPI_SCK** (**GPIO21**);
  - **CS**(**LOAD**) - к пину **GPIO18**;
- **ESP32H2**
  - **DataIn** (**DIN**) - к пину **FSPI_MOSI** (**GPIO25**);
  - **CLK** - к пину **FSPI_SCK** (**GPIO10**);
  - **CS**(**LOAD**) - к пину **GPIO0**;

Интерфейс **HSPI** для этих МК в среде **Arduino IDE** недоступен (смотри https://github.com/espressif/arduino-esp32/issues/7596), однако можно запустить второй **FSPI**-интерфейс; работа драйвера с двумя интефейсами тестировалась на следующих пинах второго **FSPI**-интерфейса:
- **DataIn** (**DIN**) - к пину **GPIO2**;
- **CLK** - к пину **GPIO3**;
...
- **FSPI**
  - **DataIn** (**DIN**) - к пину **MOSI** (**GPIO11**);
  - **CLK** - к пину **SCK** (**GPIO12**);
  - **CS**(**LOAD**) - к пину **GPIO9**; **ВАЖНО!!! - это не должен быть пин **GPIO10****;
- **HSPI** - этот интерфейс не имеет пинов по умолчанию, поэтому работа драйвера тестировалась на следующих пинах:
  - **DataIn** (**DIN**) - к пину **GPIO6**;
  - **CLK** - к пину **GPIO7**;
  - **CS**(**LOAD**) - к пину **GPIO8**;
...
- **SPI1**
  - **DataIn** (**DIN**) - к пину **TX1** (**GPIO15**);
  - **CLK** - к пину **SCL1** (**GPIO14**);
  - **CS**(**LOAD**) - к пину **CS1** (**GPIO13**);

======================================================

#### STM32

Библиотека тестировалась на **STM32F103C8T6** (**Blue Pill**), **stm32f401ccu6** (**Black Pill**), **stm32f411ceu6** (**Black Pill**)

Драйвер подключается к пинам:
- **SPI1**
...
Для **Blue Pill** возможно переназначение пинов для **SPI1** (но не средствами библиотеки):
  - **DataIn** (**DIN**) - к пину **MOSI1** (**PB5**);
  - **CLK** - к пину **SCK1** (**PB3**)

Для аддона **stm32duino**:
```
shMAX72xxMini<PA15, 4> display;
...
У **Black Pill** на этих же пинах сидит **SPI3**.

### Аддоны, используемые для работы со сторонними платами
...
*Для работы с МК **ESP32C6**, **ESP32H2** требуется версия аддона не ниже **3.0**.*
