...
- возможность изменения параметров **SPI** для работы драйвера; поддержка **SPI**-транзакций, т.е. ваше устройство не будет мешать работать другим устройства, подключенным к этому же интерфейсу, но имеющим другие настройки;
...
### История версий
Версия 1.5 - 
+ добавлена возможность изменения интерфейса **SPI** для вывода данных для **RP2040**;
+ добавлен метод `void setSPI(shSPIClass *spi);`, позволяющий выбрать интерфейс **SPI** для вывода данных; метод доступен для **ESP32**, **RP2040**;
+ добавлена возможность изменять пины интерфейса **SPI** для подключения матрицы; чтобы узнать, какие пины могут быть назначены для того или иного интерфейса, обращайтесь к описанию своего микроконтроллера;
+ добавлен метод `void init(int8_t sck_pin, int8_t mosi_pin, int8_t miso_pin = -1);`, позволяющий задать соответствующие пины текущего интерфейса **SPI**; метод доступен для **ESP32**, **ESP8266**, **RP2040**;
...
+ добавлены примеры одновременного использования двух **SPI**-интерфейсов на примере одновременного вывода двух бегущих строк; примеры написаны для **ATmega328pb**, **ESP32** и **RP2040**;
...
`void init(uint8_t spi = PB_SPI0)` -  - инициализация матрицы для **ATmega328pb** с возможностью выбора интерфейса для вывода данных; здесь:
- `spi` - индекс интерфейса; возможные варианты: **PB_SPI0** - интерфейс **SPI**, **PB_SPI1** - интерфейс **SPI1**.
...
`void init(int8_t sck_pin, int8_t mosi_pin, int8_t miso_pin = -1)` - инициализация матрицы с переназначением пинов **SPI**-интерфейса для подключения устройства; здесь:
- `sck_pin` - пин для подключения вывода **SLK** устройства;
- `mosi_pin` - пин для подключения вывода **DIN** устройства;
- `miso_pin` - пин **MISO** выбранного интерфейса; этот параметр обязателен при использовании **ESP8266** и **ESP32** в связи с особенностями реализации инициализации **SPI**-интерфейсов на этих МК; 

***Важно!!!**: для **ESP8266** можно выбрать только строго определенные пины, отличные от пинов по умолчанию: `sck_pin` - **GPIO6**, `mosi_pin` - **GPIO8**, `miso_pin` - **GPIO7**; вывод **CS** устройства при этом должен подключаться к пину **GPIO0**;*
...
`void setSPISettings(uint32_t clock, uint8_t bitOrder, uint8_t dataMode)` - настройка параметров **SPI** для работы драйвера; здесь:
- `clock` - частота синхронизации **SPI**, Гц; 
- `bitOrder` - порядок следования бит при передаче, возможные варианты - **MSBFIRST**, **LSBFIRST**;
- `dataMode` - режим передачи данных, возможные варианты - **SPI_MODE0**, **SPI_MODE1**, **SPI_MODE2**, **SPI_MODE3**;
...
На данный момент библиотека тестировалась в работе на **ATmega168p/328p/328pb**, **esp8266**, **esp32**, **esp32c3**, **rp2040** (**Zero** и **Pico**), **stm32f103c8t6** (**Blue Pill**). 

Работа с интерфейсом, отличным от **SPI0**, тестировалась на **ATmega328pb**, **esp8266**, **esp32**, **esp32c3**, **rp2040**, **stm32f103** (**Blue Pill**). 

Переназначение пинов **SPI**-интерфейса протестировано на **esp8266**, **esp32**, **esp32c3**, **rp2040**, **stm32f103** (**Blue Pill**).
...
Для работы матрицы на интерфейсе **SPI1** (с выбором этого интерфейса, см. описание метода `init(uint8_t spi = PB_SPI0)`) нужно подключить ее к cоответствующим пинам:
...
Матрица подключается к пинам **HSPI** модуля, а именно:
...
Для работы с **ESP32** матрица подключается к пинам:
- **VSPI**
...
Для работы с **ESP32C3** матрица подключается к пинам **SPI** модуля, а именно:
...
Работа на **ESP32S2** и **ESP32S3** не тестировалась, но причин для неработоспособности не видно )

#### RP2040

Протестировано подключение по умолчанию, без переназначения пинов.

Для работы с **RP2040** матрица подключается к пинам:
- **SPI0**
...
Протестировано подключение по умолчанию, без переназначения пинов. Библиотека тестировалась на **STM32F103C8T6** (**Blue Pill**)

Для работы с **Blue Pill** матрица подключается к пинам:
- **SPI1**
  - **DataIn** (**DIN**) - к пину **MOSI1** (**A7**);
  - **CLK** - к пину **SCK1** (**A5**);
  - **CS**(**LOAD**) - к пину **NSS1** (**A4**);
