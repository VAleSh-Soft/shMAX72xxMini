...
`void init(uint8_t spi = SPI_SPI0)` -  - инициализация матрицы для **ATmega328pb** с возможностью выбора интерфейса для вывода данных; здесь:
...
Работа с интерфейсом, отличным от **SPI0**, тестировалась на **ATmega328pb**, **esp32**, **esp32c3**, **rp2040**, **stm32f103** (**Blue Pill**). 

Переназначение пинов **SPI**-интерфейса протестировано на **esp32**, **esp32c3**, **rp2040**.
...
Для работы матрицы на интерфейсе **SPI1** (с выбором этого интерфейса, см. описание метода `init(uint8_t spi = SPI_SPI0)`) нужно подключить ее к cоответствующим пинам:
...
Для **Blue Pill** при использовании аддона **stm32duino** возможно переназначение пинов для **SPI1**:
